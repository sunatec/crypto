...
## 8b. M8 可读性：现状研判改作者口吻叙述
- [x] 8b.1 renderAssessment：①②③④ 保留标题当锚点，正文改**流畅段落**（Q1=b/Q2=b）
- [x] 8b.2 renderBounceHypotheses 叙述化：把参考浪/依据/之后走向揉进整句，主选就近标注、全精度数字（Q3=a/Q4=是）；
      bounceHypotheses 增补 startPrice/prevPrice 供命名参考腿，B 加 timing"时间不够"
- [x] 8b.3 近端触发线仅叙述化、保持引擎算法 61234/62321/63407（Q5=a）
- [x] 8b.4 --count 身份研判同步叙述化（renderExplicitVerdict）；76 单测全过；重跑真实报告确认可读

## 8. M7 当前反弹身份研判（作者框架）
- [x] 8.1 bounceHypotheses：XX(回撤上一子浪)/B(回撤整段0.7)/推动3(转势) 三身份，各带上方目标+依据；
      XX/B 附"跌破失效点后"的下方 Z/C 目标（Q6=b，过滤负价）；主次由反弹子浪性格定（五浪→3浪，三波→XX主B备）
- [x] 8.2 nearTermTrigger：取反弹摆动极值(57717→66924)的 0.382/0.5/0.618 作近端触发线（Q7）
- [x] 8.3 取代现状研判③的旧双情景（Q8=a）；同样接进 --count 评分节（用户结构，XX 复现作者 65766–82589）
- [x] 8.4 金标准：XX主选/B≈105722/近端从57717；--count 下 XX 复现作者区间
